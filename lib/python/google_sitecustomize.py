...
def SetupPaths():
  ...
  if _is_google3_program:
    ...
    print 'add', GOOGLEBASE
    AddPath('%s' % GOOGLEBASE)
  else:
    # non-google3

    # paths to core infrastructure libraries
    AddPath('%s/google2/pyglib' % GOOGLEBASE)

    # path needed for packages to work
    AddPath('%s' % GOOGLEBASE)

    # paths to old-style production libraries
    AddPath('%s/google/setup' % GOOGLEBASE)
    AddPath('%s/google/webserver' % GOOGLEBASE)

    # paths to crawl libraries
    AddPath('%s/google2/googlebot' % GOOGLEBASE)

    # paths to indexing libraries
    AddPath('%s/google2/indexer' % GOOGLEBASE)

    # paths to new style production libraries
    AddPath('%s/google/production/common' % GOOGLEBASE)
    AddPath('%s/google/production/babysitter' % GOOGLEBASE)
    AddPath('%s/google/production/configmgr' % GOOGLEBASE)
    AddPath('%s/google/production/fixer' % GOOGLEBASE)
    AddPath('%s/google/production/machinedb' % GOOGLEBASE)
    AddPath('%s/google/production/mdb' % GOOGLEBASE)
    AddPath('%s/google/production/assigner' % GOOGLEBASE)
    AddPath('%s/google/production/crawl' % GOOGLEBASE)

    # paths to incremental libraries
    AddPath('%s/incremental/scripts' % GOOGLEBASE)
    AddPath('%s/google2/incremental/scripts' % GOOGLEBASE)
    AddPath('%s/incremental/crawler.config' % GOOGLEBASE)
    AddPath('%s/google2/incremental/scripts/crawler.config' % GOOGLEBASE)

    # path to GEMS
    AddPath('%s/google/monitoring/gems' % GOOGLEBASE)

    # special paths for crawlmaster (remove when the crawl code is cleaned
    # up to use the standard google/google2 path)
    AddPath('%s/googlebot' % GOOGLEBASE)
    AddPath('%s/setup' % GOOGLEBASE)
    AddPath('%s/webserver' % GOOGLEBASE)

    # For swig wrappers
    AddPath('%s/google2/genfiles' % GOOGLEBASE)
    # For pywrapgooglec
    AddPath('%s/google2/bin' % GOOGLEBASE)

    # For users of protocol-buffers.
    AddPath('%s/google2/io' % GOOGLEBASE)

    # Enterprise paths
    AddPath('%s/google2/enterprise/adminrunner' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/checks' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/install' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/licensing' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/collections' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/logs' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/migration' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/net' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/scripts' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/util' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/web' % GOOGLEBASE)
    AddPath('%s/google2/enterprise/vmanager' % GOOGLEBASE)

    # Log paths
    AddPath('%s/google/analyzelogs/lib' % GOOGLEBASE)
    AddPath('%s/google/analyzelogs/scripts' % GOOGLEBASE)

    # For clients
    AddPath('%s/google2/clients' % GOOGLEBASE)

    # paths to translation console libraries
    AddPath('%s/google2/transconsole' % GOOGLEBASE)

    # paths to ads libraries
    AddPath('%s/google2/ads' % GOOGLEBASE)
